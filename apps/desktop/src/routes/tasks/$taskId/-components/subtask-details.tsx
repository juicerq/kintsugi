...
import { Plus, Check, StickyNote, FlaskConical, FileText, Code, Wrench, RefreshCw } from "lucide-react";
...
const categoryConfig: Record<SubtaskCategory, { label: string; icon: React.ReactNode; activeStyle: string }> = {
  code: { label: "Code", icon: <Code className="h-2.5 w-2.5" />, activeStyle: "bg-sky-500/20 text-sky-400" },
  test: { label: "Test", icon: <FlaskConical className="h-2.5 w-2.5" />, activeStyle: "bg-violet-500/20 text-violet-400" },
  docs: { label: "Docs", icon: <FileText className="h-2.5 w-2.5" />, activeStyle: "bg-indigo-500/20 text-indigo-400" },
  fix: { label: "Fix", icon: <Wrench className="h-2.5 w-2.5" />, activeStyle: "bg-rose-500/20 text-rose-400" },
  refactor: { label: "Refactor", icon: <RefreshCw className="h-2.5 w-2.5" />, activeStyle: "bg-emerald-500/20 text-emerald-400" },
};

const categories: SubtaskCategory[] = ["code", "test", "docs", "fix", "refactor"];
    ...
    <div className="flex flex-col gap-3 px-6 pb-3 pt-1">
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-col gap-1">
          {acceptanceCriterias.map((step, i) => (
            <div key={i} className="group/step flex items-center gap-2 rounded py-0.5">
              <button
                type="button"
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-2.5">
          <div className="flex flex-wrap items-center gap-4">
            ...
            {/* Commit */}
            <div className="flex items-center gap-2">
              <Text size="sm" variant="muted">Commit</Text>
              <Switch
