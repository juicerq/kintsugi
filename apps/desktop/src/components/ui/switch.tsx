...
>(({ className, ...props }, ref) => (
  <SwitchPrimitives.Root
    className={cn(
      ...
      "data-[state=checked]:bg-teal-500/20 data-[state=checked]:border-teal-500/30",
      className
    )}
    {...props}
    ref={ref}
  >
    <SwitchPrimitives.Thumb
      className={cn(
        "pointer-events-none block h-3 w-3 rounded-full bg-white/40 shadow-sm",
        "transition-transform data-[state=checked]:translate-x-3 data-[state=unchecked]:translate-x-0.5",
        "data-[state=checked]:bg-teal-400"
      )}
    />
  </SwitchPrimitives.Root>
));
