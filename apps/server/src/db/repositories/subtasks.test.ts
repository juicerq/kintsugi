...
import { createTestDb, createProject, createTask, createSubtask } from "../../../test/helpers";
import { createProjectsRepository } from "./projects";
import { createTasksRepository } from "./tasks";
import { createSubtasksRepository } from "./subtasks";
...
describe("SubtasksRepository", () => {
  ...
  describe("create", () => {
    ...
    test("creates subtask with all fields", async () => {
      ...
      expect(created?.acceptance_criterias).toBe('["criterion 1", "criterion 2"]');
  ...
  describe("createMany", () => {
    test("creates multiple subtasks", async () => {
      ...
      expect(created.map(s => s.name)).toEqual(["Subtask 1", "Subtask 2", "Subtask 3"]);
    });
  ...
  describe("update", () => {
    ...
    test("returns undefined when subtask not exists", async () => {
      const updated = await subtasks.update("non-existent", { status: "completed" });
